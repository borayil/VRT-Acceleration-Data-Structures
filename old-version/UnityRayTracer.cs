    ...
    public class UnityRayTracer : MonoBehaviour
    {
        ...
        public List<TreeNode<RTRay>> Render()
        {
            ...
            for (int y = 0; y < height; ++y)
            {
                for (int x = 0; x < width; ++x)
                {
                    ...
                    if (acceleratedObject)
                    {
                        ...
                        {
                            rayTree = TraceOctree(origin + pixel, pixel / pixelDistance, MaxDepth, RTRay.RayType.Normal); // Octree
                        } else
                        {
            ...
            if (accelerationOctree)
                hitTestsSaved.text = (totalTriangles - trianglesNotIgnored).ToString() + " / " + totalTriangles.ToString() + " triangles ignored";
            return rayTrees;
        }
        ...
        private TreeNode<RTRay> TraceOctree(Vector3 origin, Vector3 direction, int depth, RTRay.RayType type)
        {
            ...
            TreeNode<RTRay> rayTree = new TreeNode<RTRay>(new RTRay());
            OctreeNode ot = acceleratedObject.GetComponent<CreateOctree>().ot.rootNode;
            ...
            // Hit Octree, so check for intersection with object itself
            bool intersected = Physics.Raycast(origin, direction, out hit, Mathf.Infinity, rayTracerLayer);
        ...
        public Texture2D RenderImage()
        {
            ...
            if (acceleratedObject)
            {
                ...
                if (createOctree = acceleratedObject.GetComponent<CreateOctree>())
                {
                    octreeRoot = createOctree.ot.rootNode;
                    accelerationOctree = true;
                }
            }
